...
export default function Home() {
  ...
  const selectCat = async () => {
    try {
      ...
      if (petType !== 'cat') {
        ...
        const breedsList = await fetchBreeds(CatBaseURL, CAT_API_KEY);
  ...
  const selectDog = async () => {
    try {
      ...
      if (petType !== 'dog') {
        setIsLoading(true);
        setPetType('dog');
        const breedsList = await fetchBreeds(DogBaseURL, DOG_API_KEY);
        setBreeds(breedsList);
        setIsLoading(false);
      }
    } catch (error) {
  ...
  const selectBreed = async breedId => {
    ...
    try {
      if (petType === 'dog') {
        // console.log(`fetchPetByBreed => DOG - ${petType}`);
      ...
      if (petType === 'cat') {
        // console.log(`fetchPetByBreed => CAT - ${petType}`);
  ...
  return (
    <>
      <Buttons selectCat={selectCat} selectDog={selectDog} />
